...
def get_data():
    ...
    # Get data
    response = requests.get('https://datos.cultura.gob.ar/dataset/37305de4-3cce-4d4b-9d9a-fec3ca61d09f/resource/f7a8edb8-9208-41b0-8f19-d72811dcea97/download/salas_cine.csv')

    # If not exists, create folder
    if not os.path.exists(Path(root_folder, 'files/cines')):
        os.makedirs(Path(root_folder, 'files/cines'))

    try:
        if response.ok:
            with open(Path(
                root_folder,
                'files/cines/new_cines_data.csv'
            ), 'wb') as f:
    ...
    if not os.path.exists(Path(
        root_folder, 'datasets/cines/previous_cines_data.csv'
    )):
        # Create folder if it doesn't exists
        if not os.path.exists(Path(root_folder, 'datasets/cines')):
            os.makedirs(Path(root_folder, 'datasets/cines'))

        # Save into datasets folder.
        new_df.to_csv(
            Path(
                root_folder,
                'datasets/cines/last_cines_data.csv'),
            index=False
        )
    else:
        # Read files.
        last_df = pd.read_csv(Path(
            root_folder, 'datasets/cines/last_cines_data.csv'
        ))
        ...
        if new_data_length > previous_data_length:
            ...
            temp_merged.to_csv(Path(
                root_folder, 'datasets/cines/temp_cines_data.csv'
            ))
        else:
...
def get_cols_insterest(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cols to rename
    col_names = {'cod_localidad': 'localidad_id', 'id_provincia': 'provincia_id', 'id_departamento': 'departamento_id'}

    # Columns of interest
    cols = ['nombre', 'categoria', 'direccion', 'departamento', 'departamento_id', 'provincia', 'provincia_id', 'localidad', 'localidad_id', 'latitud', 'longitud', 'tipo_latitud_longitud', 'pantallas', 'butacas', 'espacio_incaa', 'tipo_de_gestion', 'año_actualizacion']
...
def clean_columns(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    # NORMALIZE DATA
    str_cols = ['nombre', 'categoria', 'provincia', 'localidad', 'tipo_latitud_longitud', 'tipo_de_gestion', 'tipo_de_gestion', 'direccion', 'departamento', 'espacio_incaa']
...
def transform_data():
    ...
    if not os.path.exists(Path(
        ...
        # Read csv to clean
        last = pd.read_csv('files/cines/new_cines_data.csv')
        ...
        # Save
        last.to_csv(
            'datasets/cines/previous_cines_data.csv',
            index=False
        )
        ...
        if os.path.exists(Path(
            ...
            temp_df = pd.read_csv(
                'datasets/cines/temp_cines_data.csv')
            ...
            os.remove('datasets/cines/temp_cines_data.csv')
...
def load_data():
    ...
    client.upload_file(
        r'datasets/cines/previous_cines_data.csv',
        config('BUCKET_NAME'),
        r'datasets/cines/previous_cines_data.csv'
    )
